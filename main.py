...
def parse_and_chunk(
    ...
    if parser and parser.__class__.__name__ == "ExcelParser":
        ...
        # extraction time (see services/json_parser.py) — JSON has no
        # headings or blank-line paragraphs for DocumentChunker/
        # SemanticChunker to split on, so re-running those stages here
        # would just re-slice JSON text blindly by size, cutting records
        # in half. Each returned page is already one final chunk.
        chunks = [
            Chunk(
                chunk_id=str(uuid.uuid4()),
                text=p.text,
